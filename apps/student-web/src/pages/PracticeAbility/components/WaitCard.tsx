...
export function WaitCard({ textbook, createPractice, creating = false, canCreate = true }: WaitCardProps) {
  return (
    <div className="bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all">
      <div className="p-6 flex flex-col gap-5">
        <div className="flex items-center gap-3">
          <div className="text-3xl">✨</div>
          <h3 className="text-xl font-bold text-foreground">{textbook?.semester || "能力练习"}</h3>
        </div>
        <p className="text-sm text-muted-foreground">还没有为这个能力创建练习，点击下方按钮开始吧。</p>
        <Button
