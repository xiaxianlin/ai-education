...
def get_provider(
    ...
    try:
        ...
        # Special handling for Aliyun/Dashscope to use envs directly if platform matches
        if platform.lower() in ("aliyun", "dashscope"):
            provider = AliyunProvider(
                api_key=envs.AI_PLATFORM_KEY,
                base_url=envs.AI_PLATFORM_URL
            )
        else:
            # 创建 Provider 实例
        ...
        raise ProviderConfigError(f"导入配置失败: {str(e)}")
...
def register_provider(name: str, provider_class: type):
    ...
    if not issubclass(provider_class, BaseProvider):
        raise ValueError(f"Provider 类必须继承自 BaseProvider")
