...
async def seeded_data(db_session):
    ...
    # 5. 创建会话题目关联 (通过 PracticeSessionAnswer 预置)
    a1 = PracticeSessionAnswer(session_id=session.id, question_id="q1", student_id="s1", question_order=1)
    a2 = PracticeSessionAnswer(session_id=session.id, question_id="q2", student_id="s1", question_order=2)
...
async def test_complete_practice(db_session, seeded_data):
    ...
    # 模拟已答题
    stmt = select(PracticeSessionAnswer).where(PracticeSessionAnswer.session_id == session.id)
    ...
    # 验证报告
    report = await db_session.get(PracticeSessionReport, report_id)
