...
@teacher_book_router.post("/", tags=["教师用书管理"], summary="创建教师用书", description="录入新的教师用书基本信息")
...
@teacher_book_router.post("/{id}/upload", tags=["教师用书管理"], summary="上传教师用书文件", description="上传教师用书对应的 PDF 或其他格式资源文件")
...
@teacher_book_router.put("/{id}", tags=["教师用书管理"], summary="修改教师用书信息", description="更新已录入教师用书的元数据")
...
@teacher_book_router.delete("/{id}", dependencies=[Depends(check_super_permission)], tags=["教师用书管理"], summary="删除教师用书", description="删除指定的教师用书记录及其资源")
async def delete_teacher_book(id: int, db: AsyncSession = Database):
    await teacher_book.delete_teacher_book(db, id)


@teacher_book_router.get("/search", tags=["教师用书管理"], summary="搜索教师用书", description="根据学科和年级查询教师用书")
...
@teacher_book_router.get("/{id}", tags=["教师用书管理"], summary="获取教师用书详情", description="获取指定教师用书的所有详细信息")
