...
@teacher_book_router.put(
    "/{id}", tags=["教师用书管理"], summary="修改教师用书信息", description="更新已录入教师用书的元数据"
)
...
@teacher_book_router.delete(
    "/{id}", tags=["教师用书管理"], summary="删除教师用书", description="删除指定的教师用书记录及其资源"
)
...
@teacher_book_router.get(
    "/search", tags=["教师用书管理"], summary="搜索教师用书", description="根据学科和年级查询教师用书"
)
...
@teacher_book_router.get(
    "/{id}", tags=["教师用书管理"], summary="获取教师用书详情", description="获取指定教师用书的所有详细信息"
)
