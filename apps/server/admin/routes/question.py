...
@question_router.post("/type", tags=["题型管理"], summary="创建题型", description="创建一种新的题目类型")
...
@question_router.patch("/type/{id}", tags=["题型管理"], summary="更新题型", description="更新指定题型的信息")
...
@question_router.delete("/type/{id}", tags=["题型管理"], summary="删除题型", description="删除指定的题型")
async def delete_question_type(id: int, db: AsyncSession = Database):
    await question_type.delete_question_type(db, id)


@question_router.get("/type/search", tags=["题型管理"], summary="搜索题型", description="根据条件搜索题型列表")
...
@question_router.get("/type/{id}", tags=["题型管理"], summary="获取题型详情", description="获取指定题型的详细信息")
...
@question_router.post("/", tags=["题目管理"], summary="创建题目", description="手动创建一道新题目")
...
@question_router.patch("/{id}", tags=["题目管理"], summary="更新题目", description="更新指定题目的信息")
...
@question_router.delete("/{id}", tags=["题目管理"], summary="删除题目", description="删除指定的题目")
async def delete_question(id: str, db: AsyncSession = Database):
    await question.delete_question(db, id)


@question_router.post("/{id}/image_generate", tags=["题目管理"], summary="为题目生成图片", description="调用 AI 为指定题目生成配图")
async def generate_image(id: str, db: AsyncSession = Database):
    await question.generate_question_image(db, id)


@question_router.post("/{id}/audio_generate", tags=["题目管理"], summary="为题目生成语音", description="调用 AI 为指定题目生成朗读音频")
async def generate_audio(id: str, db: AsyncSession = Database):
    await question.generate_question_audio(db, id)


@question_router.get("/search", tags=["题目管理"], summary="搜索题目", description="根据条件搜索题目列表")
