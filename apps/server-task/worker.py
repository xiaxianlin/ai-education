...
使用方法:
    python worker.py                    # 使用默认队列
    python worker.py --queue default    # 指定队列名称
    python worker.py --queue high-priority --burst  # 使用突发模式
"""
...
def start_worker(queue_name: str = None, burst: bool = False):
    ...
    if queue_name is None:
        queue_name = envs.TAKS_QUEUE_NAME
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python worker.py                          # 使用默认队列持续运行
  python worker.py --queue default          # 监听 default 队列
  python worker.py --queue high-priority    # 监听 high-priority 队列
  python worker.py --burst                  # 突发模式，处理完任务后退出
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"队列名称（默认: {envs.TAKS_QUEUE_NAME}）"
    )
