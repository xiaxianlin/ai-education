...
import { useAbilityPracticeModel } from '../models/abilityPractice';

export default function AbilityPracticeListView() {
  const navigate = useNavigate();
  const { actionRef, subject, handleDelete, importing, importQuestionTypes, validateFile } = useAbilityPracticeModel();
  ...
  const columns = useMemo<ProColumns<QuestionType>[]>(
    () => [
      ...
      createActionColumn<QuestionType>(
        (record) => (
          <>
            <Button key="edit" type="link" onClick={() => navigate(`/question_type/ability/form?id=${record.id}`)}>
              编辑
            </Button>
  ...
  return (
    <>
      ...
      <ProTable<QuestionType>
        ...
        headerTitle={
          <Space>
            <Button type="primary" size="large" onClick={() => navigate('/question_type/ability/form')} icon={<PlusOutlined />}>
              新增题型
            </Button>
        ...
        request={async ({ current, pageSize }) => {
          const res = await QuestionApi.searchAbilityPracticeTypes({
            page: current,
            size: pageSize,
            subject,
          });
