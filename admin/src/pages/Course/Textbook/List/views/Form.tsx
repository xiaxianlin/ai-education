import React, { useMemo } from 'react';
import { Form } from 'antd';
import { ModalForm, ProFormSelect } from '@ant-design/pro-components';
import { GRADES, SEMETERS, STAGES } from '../../constants';
import { useTextbookListModel } from '../models/page';

export default function TextbookForm() {
  const { form, visible, isEdit, subjects, versions, handleCancel, handleSubmit } = useTextbookListModel();

  const stage = Form.useWatch('stage', form);
  const grades = useMemo(() => (stage ? GRADES[stage] : []), [stage]);

  return (
    <ModalForm<TextbookFormModel>
      width={450}
      form={form}
      open={visible}
      title={isEdit ? '更新教材' : '新增教材'}
      onFinish={handleSubmit}
      modalProps={{
        destroyOnClose: true,
        onCancel: handleCancel,
      }}
    >
      <ProFormSelect
        name="subject"
        label="科目"
        placeholder="请选择科目"
        rules={[{ required: true }]}
        valueEnum={subjects?.reduce((prev, curr) => ({ ...prev, [curr.name]: curr.name }), {})}
      />
      <ProFormSelect
        name="version"
        label="版本"
        placeholder="请选择版本"
        rules={[{ required: true }]}
        valueEnum={versions?.reduce((prev, curr) => ({ ...prev, [curr.name]: curr.name }), {})}
      />
      <ProFormSelect
        name="stage"
        label="阶段"
        placeholder="请选择阶段"
        rules={[{ required: true }]}
        valueEnum={STAGES?.reduce((prev, curr) => ({ ...prev, [curr]: curr }), {})}
      />
      <ProFormSelect
        name="grade"
        label="年级"
        placeholder="请选择年级"
        rules={[{ required: true }]}
        valueEnum={grades?.reduce((prev, curr) => ({ ...prev, [curr]: curr }), {})}
      />
      <ProFormSelect
        name="semester"
        label="学期"
        placeholder="请选择学期"
        rules={[{ required: true }]}
        valueEnum={SEMETERS?.reduce((prev, curr) => ({ ...prev, [curr]: curr }), {})}
      />
    </ModalForm>
  );
}
