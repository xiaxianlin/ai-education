...
K12 教育辅导工具的 Monorepo 项目，使用 pnpm workspace 和 Turborepo 进行管理。
...
- **apps/admin-web**: 管理后台前端 (React 18 + Rsbuild + Ant Design 5)
  ...
  - HTTP 客户端: 基于 `@ai-education/shared-web` 的 `ApiClient`
  - 路由: react-router-dom
...
- **apps/server**: 服务端单体 (Python 3.12 + FastAPI)
  ...
  - 认证: JWT (PyJWT), AI 平台: 阿里云百炼AI, 对象存储: 阿里云 OSS
  - 日志: Loguru, 包管理器: uv
...
- **Node.js**: 使用 pnpm（pnpm@9.x，Node >= 18）
- **Python**: 使用 uv 进行依赖管理
