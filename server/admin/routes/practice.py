...
@practice_router.get("/{student_id}/daily", response_model=PracticeStatsSchem)
async def get_daily_practice(student_id: str, db: AsyncSession = Database):
    ...
    Returns:
        练习统计信息
    """
    result = await practice_service.get_daily_practice(db, student_id)
    if not result:
        raise ValueError("当天没有每日练习")
    return result


@practice_router.post("/{student_id}/daily/create", response_model=PracticeStatsSchem)
async def create_daily_practice(student_id: str, db: AsyncSession = Database):
    """
    根据学生ID生成每日练习
...
@practice_router.post("/{student_id}/daily/regenerate", response_model=PracticeStatsSchem)
async def regenerate_daily_practice(student_id: str, db: AsyncSession = Database):
    """
    根据学生ID重新生成每日练习
...
@practice_router.get("/{student_id}/history/{practice_type}", response_model=List[PracticeHistorySchema])
async def get_practice_history(student_id: str, practice_type: str, db: AsyncSession = Database):
    ...
    return await practice_service.get_practice_history(db, student_id, practice_type, limit=30)
