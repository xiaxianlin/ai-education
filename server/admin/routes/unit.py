...
@router.post("/")
async def create_course_unit(create: CourseUnitCreateSchema, db: AsyncSession = Database):
    """创建课程单元"""
...
async def delete_course_unit(unit_id: int, db: AsyncSession = Database):
    """删除课程单元"""
...
@router.get("/search")
async def list_course_units(params: SearchSchema = Depends(), db: AsyncSession = Database):
    """获取课程单元列表"""
...
async def get_questions(id: int, page: int = 1, size: int = 10, db: AsyncSession = Database):
    """获取PDF处理状态"""
    data = await QuestionService.get_by_course_unit(db, id, page, size)
    return ResponseSchema(data=data)


@router.get("/{unit_id}")
async def get_course_unit(unit_id: int, db: AsyncSession = Database):
    """获取单个课程单元"""
