...
logger = get_logger("DOUBAO_ASR_SDK")
# 常量定义
...
class Config:
    def __init__(self):
        ...
        self.auth = {"app_key": "xxxxxxx", "access_key": "xxxxxxxxxxxx"}
...
class AsrWsClient:
    ...
    async def create_connection(self) -> None:
        ...
        try:
            self.conn = await self.session.ws_connect(self.url, headers=headers)  # 使用self.session
...
async def main():
    ...
    parser.add_argument(
        "--seg-duration", type=int, default=200, help="Audio duration(ms) per packet, default:200"
    )
    ...
    async with AsrWsClient(args.url, args.seg_duration) as client:  # 使用async with
        try:
            async for response in client.execute(args.file):
                logger.info(
                    f"Received response: {json.dumps(response.to_dict(), indent=2, ensure_ascii=False)}"
                )
        except Exception as e:
