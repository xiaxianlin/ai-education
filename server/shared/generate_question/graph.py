...
class QuestionGenerationState(TypedDict):
    ...
    count: int  # 数量
    generation_type: str  # 生成方式： daily_practice/unit_practice/assessment/unit/textbook
    unit: Any
    textbook: Any
    knowledge_text: str
    knowledge: str
...
async def check_params_node(state: QuestionGenerationState) -> QuestionGenerationState:
    ...
    if state["generation_type"] not in ["practice", "normal"]:
        raise ValueError("生成方式必须为 practice 或 normal")
    if state["practice_type"] not in ["daily", "unit", "assessment"]:
        raise ValueError("练习类型必须为 daily、unit 或 assessment")


async def router_node(state: QuestionGenerationState):
    """根据生成方式路由到不同的节点"""
    return state["generation_type"]


async def load_textbook_data_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载教材数据"""
    return state


async def load_unit_data_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载单元数据"""
    return state


async def load_unit_practice_data_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载单元练习数据"""
    return state


async def load_daily_practice_data_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载今日练习数据"""
    return state


async def load_assessment_data_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载能力评测数据"""
    return state


async def load_textbook_prompt_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载教材 prompt"""
    return state


async def load_unit_prompt_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载单元 prompt"""
    return state


async def load_unit_practice_prompt_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载单元练习 prompt"""
    return state


async def load_daily_practice_prompt_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载今日练习 prompt"""
    return state


async def load_assessment_prompt_node(state: QuestionGenerationState) -> QuestionGenerationState:
    """加载能力评测 prompt"""
    return state
...
def create_question_generation_graph() -> StateGraph:
    ...
    workflow.add_node("load_textbook_prompt", load_textbook_prompt_node)
    workflow.add_node("load_unit_prompt", load_unit_prompt_node)
    workflow.add_node("load_unit_practice_prompt", load_unit_practice_prompt_node)
    workflow.add_node("load_daily_practice_prompt", load_daily_practice_prompt_node)
    workflow.add_node("load_assessment_prompt", load_assessment_prompt_node)
    ...
    workflow.add_conditional_edges(
        ...
        [
            "load_textbook_data",
            "load_unit_data",
            "load_unit_practice_data",
            "load_daily_practice_data",
            "load_assessment_data",
        ],
    )

    # 加载数据节点完成后，进入加载 prompt 节点
    workflow.add_edge("load_textbook_data", "load_textbook_prompt")
    workflow.add_edge("load_unit_data", "load_unit_prompt")
    workflow.add_edge("load_unit_practice_data", "load_unit_practice_prompt")
    workflow.add_edge("load_daily_practice_data", "load_daily_practice_prompt")
    workflow.add_edge("load_assessment_data", "load_assessment_prompt")

    # 加载 prompt 节点完成后，进入调用大模型节点
    workflow.add_edge("load_textbook_prompt", "call_llm")
    workflow.add_edge("load_unit_prompt", "call_llm")
    workflow.add_edge("load_unit_practice_prompt", "call_llm")
    workflow.add_edge("load_daily_practice_prompt", "call_llm")
    workflow.add_edge("load_assessment_prompt", "call_llm")
