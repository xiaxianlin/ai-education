...
class RedisSemanticCache:
    ...
    def __init__(
        ...
        self.logger = get_logger("RedisSemanticCache")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    json={"model": self.model_name, "input": text},  # 使用 input 而不是 prompt
                ) as response:
                    result = await response.json()
                    # Ollama embed API 返回格式为 {"embeddings": [[...], ...]}
                    return result["embeddings"][0]  # 返回第一个向量
        except Exception as e:
            self.logger.error(f"Error getting Ollama embedding: {str(e)}", exc_info=True)
            raise
    ...
    async def _get_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            self.logger.error(f"Error in get_embedding: {str(e)}", exc_info=True)
            raise
        ...
        while True:
            try:
                ...
                if len(all_keys) > self.max_cache_size:
                    ...
                    for key in all_keys:
                        metadata = json.loads(
                            self.redis.get(key.encode("utf-8")).decode("utf-8")
                        )  # 编码key再获取
                ...
                self.logger.info(f"Cache cleanup completed for prefix {self.prefix}")
    ...
    async def _remove_cache_item(self, hash_id: str):
        ...
        try:
            ...
            self.logger.error(f"Error removing cache item: {str(e)}", exc_info=True)
    ...
    async def lookup(self, messages: List[Dict]) -> Optional[str]:
        ...
        try:
            ...
            for vec_key in all_vectors:
                cached_vector = json.loads(
                    self.redis.get(vec_key.encode("utf-8")).decode("utf-8")
                )  # 编码key再获取
            ...
            if max_similarity >= self.score_threshold and most_similar_key:
                ...
                if cached_response:
                    ...
                    self.logger.info(f"Cache hit with similarity: {max_similarity:.4f}")
                    return cached_response.decode("utf-8")

            return None

        except Exception as e:
            self.logger.error(f"Error in lookup: {str(e)}", exc_info=True)
            return None
    ...
    async def update(self, messages: List[Dict], response: str, expire: int = None):
        ...
        try:
            ...
            self.logger.info(f"Cache updated for message: {user_message[:50]}...")
