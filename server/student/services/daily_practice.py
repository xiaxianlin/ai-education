...
async def check_daily_practice(
    db: AsyncSession, student_id: str
) -> Optional[PracticeStatsSchem]:
    """检查是否有当天的每日练习"""
...
async def reset_daily_practice(
    db: AsyncSession, student_id: str, session: PracticeSession
) -> None:
    """重置往期未完成的每日练习为当天的练习"""
    current_date = today()
    ...
    try:
        ...
        logger.info(
            f"往期练习已重置为当天: session_id={session.id}, new_date={current_date}"
        )
