...
class UserQuestionService:
    ...
    @staticmethod
    async def get_questions(db: AsyncSession, user_id: str, params: QuestionRequestSchema) -> QuestionBatchSchema:
        """获取问题（优先从数据库，不足时AI生成）"""
        ...
        questions_data = [QuestionSchema.model_validate(q).model_dump() for q in existing_questions[:params.count]]
    ...
    @staticmethod
    async def _query_existing_questions(db: AsyncSession, params: QuestionRequestSchema) -> List[Question]:
        """查询现有问题"""
    ...
    @staticmethod
    async def _start_generation_task(db: AsyncSession, params: QuestionRequestSchema, count: int) -> str:
        """启动AI问题生成任务"""
        ...
        if params.course_unit_id:
            unit = await db.scalar(
                select(CourseUnit).where(CourseUnit.id == params.course_unit_id)
            )
            if unit:
    ...
    @staticmethod
    async def _generate_questions_task(params: Dict[str, Any], knowledge_content: str, unit_content: str, count: int):
        """AI问题生成后台任务"""
