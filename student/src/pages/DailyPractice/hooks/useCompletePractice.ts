...
import { practiceApi, DailyPracticeReport } from '@/services/practice';
import { toast } from 'sonner';

export function useCompletePractice() {
  const [submitting, setSubmitting] = useState(false);
  const [report, setReport] = useState<DailyPracticeReport | null>(null);
  ...
  const completePractice = useCallback(async (sessionId: number, unansweredCount: number) => {
    ...
    try {
      setSubmitting(true);
      const result = await practiceApi.completeDailyPractice(sessionId);
      setReport(result);
      return result;
    } catch (error) {
