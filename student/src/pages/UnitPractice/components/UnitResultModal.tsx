...
import { Trophy, Award, Clock, CheckCircle2, TrendingUp, Star, Sparkles, Home, History } from 'lucide-react';
          ...
          <CardContent className="py-4 px-6">
            <div className="flex items-center justify-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-semibold text-purple-700 mt-1">{report.unit_name}</p>
                <p className="text-sm text-pink-600 font-medium mt-0.5">{getEncouragementText()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6 pb-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center transform transition-transform hover:scale-105 duration-200">
                ...
                <div className="text-4xl font-bold text-blue-600 mb-1">{report.score.toFixed(0)}</div>
              ...
              <div className="text-center transform transition-transform hover:scale-105 duration-200">
                ...
                <div className="text-4xl font-bold text-green-600 mb-1">
                  {report.correct_questions}<span className="text-2xl text-gray-400">/{report.total_questions}</span>
                </div>
                <div className="text-sm text-gray-600 font-medium">正确题数</div>
              </div>
              <div className="text-center transform transition-transform hover:scale-105 duration-200">
              ...
              <div className="text-center transform transition-transform hover:scale-105 duration-200">
                ...
                <div className="text-4xl font-bold text-orange-600 mb-1">
                  {Math.floor(report.total_time / 60)}:{(report.total_time % 60).toString().padStart(2, '0')}
                </div>
        ...
        <div className="animate-in slide-in-from-bottom duration-500 delay-300">
          ...
          {Object.keys(report.knowledge_scores).length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.entries(report.knowledge_scores).map(([knowledge, score], index) => (
                <Card 
                  key={knowledge} 
                  className={cn(
                  ...
                  style={{
                    animation: `slideIn 0.5s ease-out ${0.4 + index * 0.1}s backwards`
                  }}
                >
                  <CardContent className="pt-4 pb-4">
                    <div className="text-center space-y-3">
                      ...
                      <div className={cn(
                        "text-4xl font-bold",
                        score.rate >= 0.8 ? "text-green-600" : score.rate >= 0.6 ? "text-yellow-600" : "text-red-600"
                      )}>
                      ...
                      <div className={cn(
                        ...
                        <span>{score.correct}/{score.total} 题</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
