...
export const dailyPracticeApi = {
  ...
  getHistory: async (limit?: number): Promise<DailyPracticeHistoryItem[]> => {
    ...
    return api.get<DailyPracticeHistoryItem[]>(`/practice/history/daily_practice${params}`);
  },
  ...
  checkToday: async (): Promise<{
    ...
    try {
      ...
      return {
        session,
        task_id: null,
        status: session ? (session.status === 2 ? 'completed' : session.status === 1 ? 'in_progress' : 'pending') : 'failed',
   ...
   * 使用统一的会话详情接口
   */
  getSession: async (sessionId: number): Promise<{ session: PracticeSession; questions: any[] }> => {
    // 使用统一的会话详情接口
  ...
  getStats: async (): Promise<{
    ...
    return {
      today_progress: dailyQuestions > 0 ? (completedQuestions / dailyQuestions) * 100 : 0,
